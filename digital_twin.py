...
def clusters(nodes):
    ...
    remaining={n['node_id']:n for n in nodes if n['risk_level'] in ('MEDIUM','HIGH','CRITICAL') and n['status']=='ONLINE'}
    result=[]
    while remaining:
...
def build_twin_state(nodes, readings, alert, now=None):
    ...
    values=[]
    for node in nodes:
        rd=lookup.get(node['node_id'],{});age=age_seconds(rd.get('timestamp'),now)
    ...
    level=min(1,max([max(math.hypot(n['tilt_x'],n['tilt_y'])/3,n['displacement_mm']/15) for n in affected] or [0]))
    crack=max([n['displacement_mm'] for n in values if n['node_type']=='crack' and n['status']=='ONLINE'] or [0])
    return dict(version=1,source='HOST_DATABASE',input_mode='SIMULATION',generated_at=now.isoformat(),system_state=state,
