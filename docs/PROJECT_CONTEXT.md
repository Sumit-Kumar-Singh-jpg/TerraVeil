...
## Live InSAR ground-response illustration (2026-09-17)

Implemented against the current TerraVeil 1.1 working copy after concurrent edits were paused. This checkout no longer contains the earlier SSE/live-node module; do not assume that historical implementation is present. The current dashboard polls `/api/live` every two seconds and now shares that snapshot via `terraveil:telemetry` and `window.TerraVeilTelemetry`. `static/js/ground-motion.js` consumes its registered node IDs, coordinates, readings and reported mode without another polling loop. Existing simulator/database edits from the other task were preserved.

The InSAR view renders sensor markers and a separate orange wireframe for the selected node. Fresh tilt/displacement changes smoothly animate this patch in settlement or illustrative downslope-slide mode. Markers rotate with tilt, vibration pulses rings, live risk sets marker colours, and Focus node brings the local patch into view. Historical DEM positions, InSAR LOS values, sample inspection and recommendation coordinates are unchanged. The patch masks missing DEM triangles and nodes outside the raster stay unlocated in the panel. Cyan diamonds remain recommendations; readings are not assigned to recommendations by array index or proximity.

This is an explicitly labelled **illustration**, not a physical deformation reconstruction. The first fresh readings on page load/source/session/position change establish a temporary reference. A local button resets that reference without altering backend calibration or data. The artistic response is bounded: tilt change above 0.05 degrees scales toward full response at 5 degrees, or gauge change toward full response at 10 mm. Vibration alone never creates settlement. A fixed 700 m influence radius and up to 0.18 scene-km vertical / 0.22 scene-km downslope offsets (times user visual gain) are display parameters, not measured distances, depths or affected areas. Downslope direction comes from the DEM, not MPU heading. On flat/unknown slope no horizontal direction is invented. Changing a crack gauge does not prove vertical settlement. Measured depth and slide distance remain unavailable.

The original terrain remains visible beneath a wireframe drawn without depth occlusion. The selected patch interpolates between fresh readings; offline backend readings or a six-second feed interruption freeze its last shape, mute it, and stop vibration. Source switching clears shapes and references. Coordinates remain fixed. The existing backend freshness timeout and ingestion/risk logic are unchanged. Backend mode labels are passed through as reported; any separately enabled hardware-packet generator needs its own provenance handling and is not physical evidence.

Validation: 65 Python tests passed with a disposable database and serial disabled. `node tests/test_ground_motion.mjs` passes signal/baseline/units, fresh animation, fixed coordinates, unchanged historical geometry, offline freeze, source reset, node removal and cleanup checks. Changed JavaScript syntax checks pass. Browser simulation checks confirmed changing tilt, visible selected-node wireframe, focus and slide controls. No physical hardware validation or calibrated depth reconstruction is claimed.

Browser verification also confirmed REAL mode with no fresh readings displays unavailable values and freezes animation. The running host on port 5000 serves the updated frontend; at verification it reported COM7 disconnected and all three REAL nodes offline. A pre-existing syntax error in `digital-twin.js` was observed and left outside this InSAR change.
